...
（3）本机在浏览器输入`http://localhost`可访问 openQA web ui，局域网其他机器在浏览器通过服务器IP地址同样可以访问
...
（6）拷贝新建的key和secret，在安装openQA的服务器上编辑 /etc/openqa/client.conf 文件，在如下位置填入key和secret
...
$ git clone https://intranet.leapfive.com/bitbucket/scm/dcsdkdo/os-autoinst-distri-openeuler-riscv.git
